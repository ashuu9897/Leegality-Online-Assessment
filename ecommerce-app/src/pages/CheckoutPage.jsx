...
const emptyForm = { name: '', email: '', phone: '', address: '', city: '', zip: '' };
...
export default function CheckoutPage() {
  ...
  const unit = (item) => item.price * (1 - (item.discountPercentage || 0) / 100);
  const subtotal = items.reduce((s, i) => s + unit(i) * i.qty, 0);
  const savings = items.reduce(
    (s, i) => s + (i.price - unit(i)) * i.qty,
    0
  );
    ...
    return (
      <main className="max-w-2xl mx-auto px-4 py-12">
        <div className="bg-white rounded-lg border border-gray-200 p-8 text-center">
          <CheckCircleIcon sx={{ fontSize: 64 }} className="text-green-600 mb-3" />
          ...
          <div className="mt-6 bg-gray-50 border border-gray-200 rounded-lg p-4 text-left text-sm space-y-1.5">
            ...
            <div className="flex justify-between">
              ...
              <span className="text-green-700 font-medium">{deliveryDateLabel()}</span>
            </div>
          </div>
  ...
  if (items.length === 0) {
    return (
      <main className="max-w-2xl mx-auto px-4 py-16 text-center">
        <ShoppingBagOutlinedIcon sx={{ fontSize: 64 }} className="text-gray-300 mb-3" />
        ...
        <div className="flex-1 w-full space-y-6">
          <section className="bg-white rounded-lg border border-gray-200 p-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <Field label="Phone" value={form.phone} onChange={update('phone')} error={errors.phone} />
              <Field label="ZIP / Postal code" value={form.zip} onChange={update('zip')} error={errors.zip} />
              ...
              {[
                ...
                <label
                  ...
                  className={`flex items-center gap-3 border rounded-lg px-4 py-3 cursor-pointer transition-colors ${
                    payment === opt.id ? 'border-[#e77600] bg-orange-50' : 'border-gray-200 hover:bg-gray-50'
                  }`}
                >
                  <input
                    type="radio"
            ...
            <ul className="divide-y divide-gray-100 max-h-64 overflow-y-auto -mx-1 px-1">
              {items.map((item) => (
                <li key={item.id} className="flex gap-3 py-3">
                  <img src={item.thumbnail} alt="" className="w-12 h-12 object-contain bg-gray-50 rounded border border-gray-100 shrink-0" />
                  <div className="flex-1 min-w-0">
                    <p className="text-xs text-gray-700 line-clamp-2">{item.title}</p>
                    <p className="text-xs text-gray-500">Qty: {item.qty}</p>
                  </div>
                  <span className="text-sm font-medium text-gray-900">${(unit(item) * item.qty).toFixed(2)}</span>
                </li>
              ))}
            </ul>
